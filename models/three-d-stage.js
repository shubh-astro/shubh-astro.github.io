...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      /* `no-export` hides the download toolbar — the buttons still exist so
         _runExport and _setButtonsEnabled need no special-casing. */
      ...
      if (!this.hasAttribute('no-export')) root.appendChild(this._toolbar);
    ...
    setObject(object) {
      ...
      if (!box.isEmpty()) {
        ...
        const half =
          Math.max(size.x / Math.max(this._camera.aspect, 0.2), size.y, size.z) / 2;
        const dist =
          (half / Math.tan((this._camera.fov * Math.PI) / 360)) * 1.12;
        const dir = new THREE.Vector3(1, 0.3, 1.15).normalize();
